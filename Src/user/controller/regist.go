...
func UserRegist(c *gin.Context) {
	...
	//检查验证码是否正确
	if !dao.RegistVerifyCodeExist(dt.Id, dt.Verifycode) {
		util.ResponseNAK_MSG(c, "验证码错误", "")
		return
	}
	//查询用是否允许注册
	if !dao.UserIdLegal(dt.Id) {
	...
	if !dao.UserAdd(session, dt.Id, password, dt.Email) {
...
func UserRegistCodeSend(ctx *gin.Context) {
	...
	dt.Email = userEmailProcess(dt.Id, dt.Email)
	//判断是否可以发送验证码
	ttl := dao.RegistVerifyCodeCanSendTTL(dt.Id)
	if ttl > 0 {
	...
	//插入Redis
	success := dao.RegistVerifyCodeAdd(dt.Id, code)
	//
	if !success {
